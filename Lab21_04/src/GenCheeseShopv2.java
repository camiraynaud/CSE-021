...
public class GenCheeseShopv2 {
	...
	public static void printFinalTotal(double newSubTotal) 
	{
		...
		if(discount > 1) {
			System.out.printf("Additional 25%s Discount: %10s%.2f \n",percent, minusCash, Math.abs(discount)); //changed 10 to 25
		}
		else if(discount <= 0) {
			System.out.printf("Additional 10%s Discount: %11s%.1f \n",percent, minusCash, Math.abs(discount));
